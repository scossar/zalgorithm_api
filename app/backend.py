...
def select_fragment_ids(collection, query: str, limit: int, candidate_limit: int) -> list[int]:
    ...
    if not size:
        return []
    while True:
        result = collection.query(query_texts=[query], n_results=size, include=["metadatas"])
...
class Backend:
    def __init__(self, settings: Settings):
        ...
        try:
            ...
            if settings.snapshot is not None:
                ...
                if metadata.get("schema_version") != 1 or metadata.get("embedding_model") != "all-MiniLM-L6-v2":
                    raise ValueError("Unsupported snapshot schema or embedding model")
                if metadata.get("versions", {}).get("chromadb") != "1.5.9":
                    raise ValueError("Snapshot Chroma version must match this API (1.5.9)")
                if metadata.get("collection") != settings.collection:
                    raise ValueError("Snapshot collection differs from CHROMA_COLLECTION")
            ...
            with closing(sqlite3.connect(sqlite_uri(self.db_path), uri=True)) as db:
                db.execute("SELECT id, html_heading, html_fragment FROM sections LIMIT 1").fetchall()
                fragment_count = db.execute("SELECT count(*) FROM sections").fetchone()[0]
                if metadata and fragment_count != metadata["fragments"]:
                    raise ValueError("Snapshot SQLite count differs from build manifest")

            chroma_settings = ChromaSettings(anonymized_telemetry=False)
            if settings.snapshot is not None:
                self.client = chromadb.PersistentClient(path=str(working / "chroma"), settings=chroma_settings)
            else:
                self.client = chromadb.HttpClient(host=settings.chroma_host, port=settings.chroma_port,
                                                  ssl=settings.chroma_ssl, settings=chroma_settings)
            encoder = ONNXMiniLM_L6_V2(preferred_providers=["CPUExecutionProvider"])
            self.collection = self.client.get_collection(settings.collection, embedding_function=encoder)
            ...
            if metadata:
                ...
                if digest != metadata["tokenizer_sha256"]:
                    raise ValueError("Query tokenizer differs from the snapshot tokenizer")
                if self.collection.count() != metadata["chunks"]:
                    raise ValueError("Snapshot Chroma count differs from build manifest")
    ...
    def search(self, query: str) -> list[int]:
        ...
        return select_fragment_ids(self.collection, query, self.settings.result_limit, self.settings.candidate_limit)
