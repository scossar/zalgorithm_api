...
class Settings:
    ...
    def __post_init__(self):
        ...
        if not 1 <= self.result_limit <= self.candidate_limit <= 10000:
            raise ValueError(
                "Require 1 <= SEARCH_RESULT_LIMIT <= SEARCH_CANDIDATE_LIMIT <= 10000"
            )
    ...
    def from_env(cls):
        ...
        return cls(
            ...
            sqlite_path=Path(
                os.getenv("SQLITE_DB_PATH", "/data/sections.db")
            ).expanduser(),
            ...
            cors_origins=tuple(
                s.strip()
                for s in os.getenv("CORS_ORIGINS", ",".join(cls.cors_origins)).split(
                    ","
                )
                if s.strip()
            ),
